...
<head>
    ...
    <title>GestPAT - Liste des RDV</title>
    <meta content="Admin Dashboard" name="description">
    <meta content="Themesbrand" name="author">
    <link rel="shortcut icon" href="/images/favicon.ico">

    <!-- Plugin css -->
    <link href="<c:url value='/public/libs/fullcalendar/fullcalendar.min.css' />" rel="stylesheet" type="text/css" />
    ...
    <div class="content-page">
        ...
        <div class="content">
            <div class="container-fluid">
                ...
                <input type="hidden" id="secretaireid" value="${secretaireid}" />
                <!-- end row -->
            </div>
            <!-- container-fluid -->
        </div>
...
<script src="<c:url value='/public/libs/jquery/jquery.min.js' />"></script>
<script src="<c:url value='/public/libs/bootstrap/js/bootstrap.bundle.min.js' />"></script>
<script src="<c:url value='/public/libs/metismenu/metisMenu.min.js' />"></script>
<script src="<c:url value='/public/libs/simplebar/simplebar.min.js' />' />"></script>
<script src="<c:url value='/public/libs/node-waves/waves.min.js' />"></script>

<!-- plugin js -->
<script src="<c:url value='/public/libs/moment/min/moment.min.js' />"></script>
<script src="<c:url value='/public/libs/jquery-ui/jquery-ui.min.js' />"></script>
<script src="<c:url value='/public/libs/fullcalendar/fullcalendar.min.js' />"></script>

<!-- Calendar init -->
<script src="<c:url value='/public/js/pages/calendar.init.js' />"></script>
...
<script>
    $(document).ready(function() {
        $().ready(function () {
            ...
            var secretaireid = $("#secretaireid").val();

            $.get(monurl+"getcalendriermedecinrdv/"+service+"/"+secretaireid,
            //$.get("http://localhost:8080/gestion/getcalendriermedecinrdv/"+service,
                function(data) {
                    //
                    var patient = [];
                    var jour = [];
                    ...
                    $(data).find('item').each(function () {
                        patient.push($(this).find('patient').text());
                        jour.push(parseInt($(this).find('jour').text()));
                        mois.push(parseInt($(this).find('mois').text()));
                        heure.push(parseInt($(this).find('heure').text()));
                        minute.push($(this).find('minute').text());
                        annee.push(parseInt($(this).find('annee').text()));
                    });
                    ...
                    for(var i=0; i < patient.length; i++){
                        // Process :
                        var tampon2 = {};
                        tampon2.title = patient[i];
                        tampon2.start = new Date(annee[i], mois[i],jour[i], heure[i], minute[i]);
                        tampon2.allDay = !1;
                        tabDonnee.push(tampon2);
                    }
                    ...
                    $("#calendar").fullCalendar({
                        ...
                        drop:function(e,t){
                            var a=i(this).data("eventObject"),
                                n=i.extend({},a);
                            n.start=e,
                                n.allDay=t,i("#calendar").fullCalendar("renderEvent",n,!0),
